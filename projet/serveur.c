...
{ // la fonction ne supprime pas l'utilisateur mais passe son état à deconnecter
    utilisateurs[tid-1].connecte = FAUX; 
}

char *printTime(void)
{ // lafonction permet de retourner la date/heure sleon un format standard de time.h (utilité => log)
...
void ecrireLog(void)
{ //en tête pour les logs avec la date affichée
